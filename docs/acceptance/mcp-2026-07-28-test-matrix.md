...
| MCP-P1-01 | P1 | Draft 2020-12 input/output validation only works with an explicit compiler | `components/mcp/tests/schema/test_jsonschema_compiler.py` | Adapter contract; official full-schema suite not run (R-232 路径变更) |
| MCP-P1-02 | P1 | External `$ref` is rejected rather than fetched | same test module | Automated security boundary |
| MCP-P1-03 | P1 | ASGI maps POST, content type, MCP headers, body and JSON response without a web framework | `components/mcp/tests/transport/test_asgi_application.py` | Controlled ASGI contract; no deployed ASGI server/SSE stream exercised (R-232 路径变更) |
| MCP-P1-04 | P1 | Bearer validation produces only token-free `ToolContext`; failure emits PRM challenge | `components/mcp/tests/security/test_oauth_bridge.py` | Fake validator only; no real issuer/JWKS/introspection acceptance (R-232 路径变更) |
| MCP-P1-05 | P1 | Owned HTTP adapter mirrors method/name and protocol headers | `components/mcp/tests/transport/test_http_exchange.py` | Fake HTTP executor only; no peer server/SSE acceptance (R-232 路径变更) |
