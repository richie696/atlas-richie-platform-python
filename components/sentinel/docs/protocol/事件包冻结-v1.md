...
它是 [`事件上报协议-v1.md`](./事件上报协议-v1.md) 的 sign-off 记录, 也是
Reporter 和 Collector **必须**遵守的 schema 级合同。

本文档有意保持简短。Schema 本身, 包括所有字段定义, payload 类型, 以及
序列化规则, 在 [`事件上报协议-v1.md`](./事件上报协议-v1.md) 中规定。
外层 transport, 鉴权, batching 和 sequence 语义在
[`上报父协议-v1.md`](./上报父协议-v1.md) 中规定。本文档只记录冻结事件本身,
以及由此带来的不可变性保证。
...
被冻结的工件是 [`事件上报协议-v1.md`](./事件上报协议-v1.md) 中规定的
...
- 序列化规则 (UTF-8 字符串, ISO 8601 微秒时间, 小写 UUID, 基于 key 的
  object 解析)。
- 大小限制 (envelope ≤ 16 KB; 参见 `上报父协议-v1.md` §3.4)。
...
4. Wire 标识符字符串 (`atlas-richie.reporting/v1`) 不可变。任何改动需要
   V2 major 和一个 ADR。
5. [`事件上报协议-v1.md` §3.3](./事件上报协议-v1.md#33-serialization)
   中的序列化规则不可变。
...
| 文档 | 用途 |
| --- | --- |
| `事件上报协议-v1.md` (本协议族) | V1 envelope schema。本文冻结的工件。 |
| `上报父协议-v1.md` | 外层 transport, 鉴权, batching 和 sequence。 |
