...
- **产品最终保留** 的文档放在 `docs/` 根 (跟 `DESIGN.md`, `OPERATIONS.md`,
  `RELEASE.md` 同级); V1 协议冻结后也不带 M 编号 (例: `REPORTING-PROTOCOL.md`,
  `AGENT_REPORTING_PROTOCOL.md`, `CLUSTER_TOKEN_PROTOCOL.md`).
...
- **Final product docs** (retained) live in `docs/` root (alongside
  ...
  also use non-M-number names (e.g. `REPORTING-PROTOCOL.md`,
  `AGENT_REPORTING_PROTOCOL.md`, `CLUSTER_TOKEN_PROTOCOL.md`).
